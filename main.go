...
func main() {
	...
	flatJSON, err := json.Marshal(flatItems)
	if err != nil {
...
func getWikipediaSummary(articleTitle string) (string, error) {
	...
	apiUrl := fmt.Sprintf("https://en.wikipedia.org/api/rest_v1/page/summary/%s", url.QueryEscape(articleTitle))
	...
	summary, ok := result["extract_html"].(string)
	if !ok {
		return "", fmt.Errorf("failed to extract 'extract_html' from JSON response")
	}
